...
for path in image_paths:
    ...
    print(str(list(range(0, np.amax(ann_inst)))))
    print(str(np.unique(ann_inst).tolist()))
    assert np.unique(ann_inst).tolist() == list(range(0, np.amax(ann_inst)))
    assert np.unique(ann_type).tolist() == list(range(0, np.amax(ann_type)))
